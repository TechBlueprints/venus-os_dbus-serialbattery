...
* Added: `BLUETOOTH_CONNECTION_BACKEND` config option to select how Bluetooth LE connections are established. `BleakBackend` (the existing behavior) is the default; `BCMBackend` is an opt-in managed alternative by @cgoudie
* Added: Bluetooth LE - Opt-in `BCMBackend` connection backend, built on the bundled `bleak_connection_manager` (`dbus-serialbattery/ext/`, Apache-2.0, see its `README.md` for provenance). It resolves the device from the BlueZ cache or via `Adapter1.ConnectDevice` instead of scanning, retries with per-adapter failure tracking and an escalation ladder, cleans up phantom connections, and treats `BLUETOOTH_ADAPTERS` as a hard per-device allow-list, evicting cache entries that point at a disallowed adapter. Aimed at GX devices where several BLE services share a few adapters. Nothing changes unless `BLUETOOTH_CONNECTION_BACKEND = BCMBackend` is set, and the driver falls back to `BleakBackend` if the backend cannot be loaded on that system by @cgoudie
...
* Changed: Bluetooth LE - `BCMBackend` breaks half-connect oscillation: after 3 consecutive attempts where the device connects but the handoff to a usable notification channel fails, it pauses 10 s so the peripheral can advertise again and resolves by scan instead of re-occupying it. If the breaker engages 10 times within 5 minutes the backend writes the automatic hold flag for that battery, standing the reconnect loop down for 20 minutes of real radio quiet. Seen three times in production, each time wedging one battery for 10-26 minutes, and each time cured by exactly that quiet by @cgoudie
