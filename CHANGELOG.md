...
* Added: `FALLBACK_SENSOR_DBUS_DEVICE` and `FALLBACK_SERVE_STALE_MINUTES` to serve data from a fallback device, e.g. a SmartShunt, while the connection to the BMS is lost: voltage, current and temperature are read live from the fallback device, per-cell voltages are projected from the live shunt voltage (the disconnect snapshot shifted by the pack delta, published in place of the frozen cells), and everything not derivable is served stale (frozen at the last BMS read) instead of resetting the values and going into an error state. With the dedicated `FALLBACK_SAFE_CELL_VOLTAGE_MIN/MAX` safe zone configured (set from the battery manufacturer's spec; used ONLY during fallback, deliberately not shared with any other voltage option), charge and discharge follow the projected cell extremes: inside the zone both are allowed with the last known limits, above it charging is blocked, below it discharging is blocked, with 0.05 V/cell re-entry hysteresis. Without a configured zone or a valid projection, charging is blocked entirely and discharging stays allowed. Serving continues indefinitely while the fallback delivers live values; `FALLBACK_SERVE_STALE_MINUTES` bounds serving without live fallback data, after which the ordinary disconnect timer runs. With `FALLBACK_SENSOR_DBUS_PATH_SOC` configured, SoC is served live from the fallback device and remaining capacity, consumed amp hours, time-to-go and battery state are derived from the live fallback values. Stale serving engages unconditionally on disconnect (the BMS's own cell-level protection acts independently of the driver), and `FALLBACK_BMS_CABLE_WARN_MINUTES` delays the BmsCable warning while the fallback is actively serving, so short self-healing Bluetooth blips do not raise warnings by @cgoudie
