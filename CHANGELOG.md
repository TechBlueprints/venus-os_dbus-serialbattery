...
* Changed: enable.sh - Generate run scripts that `exec` the driver instead of starting it in the background and waiting on it. The run script forwarded a single TERM and then exited when its `wait` was interrupted, so `supervise` respawned while a driver that outlived that one signal was reparented to init still holding its D-Bus name and its Bluetooth link; the successor then failed repeatedly with "another live instance". Under `exec` the driver is the supervised process, so the signal reaches it directly and the service slot stays occupied until it really exits. This also fixes the exit status, which was reported as 128 plus the signal number rather than the driver's own by @cgoudie
* Changed: enable.sh - Raise the log retention of the battery services from `s500000 n4` (2 MB) to `s1500000 n20` (30 MB), matching what the Bluetooth sensor services already use. Retention has to be sized against the log rate during a fault, not when idle: a driver retrying a connection it cannot make logs a few lines per second, and at that rate 2 MB holds well under an hour, so the record of what started the fault is evicted before anyone looks. `s`/`n` bound what is kept, not what is written, so this costs disk space only by @cgoudie
