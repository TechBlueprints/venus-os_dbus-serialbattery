...
* Added: fallback sensor (`FALLBACK_SENSOR_DBUS_DEVICE`) to keep operating from a second instrument, e.g. a SmartShunt, whenever the BMS is not delivering: which source is published is decided every cycle by the age of the BMS data (BMS values while the link is up and the data is younger than 15 seconds, the fallback device otherwise, switching over instantly on a known disconnect), so a half-connected link or a driver restart mid-outage serves live shunt values immediately. The last real per-cell voltages are persisted as a projection *basis*, never as served values, so a process that restarts mid-outage recomputes its charge and discharge limits from the live shunt each cycle instead of serving the pack's remembered rating. Voltage, current, temperature and SoC come from the fallback device, power, remaining capacity, consumed amp hours and time-to-go are derived from them, and per-cell voltages are projected from the live shunt voltage (the last live cell reading shifted by the pack delta, refused once that reading is more than 10 minutes old). With the dedicated `FALLBACK_SAFE_CELL_VOLTAGE_MIN/MAX` safe zone configured (set from the battery manufacturer's spec; used ONLY while the fallback serves), charge and discharge follow the projected cell extremes: inside the zone both are allowed with the last known limits, above it charging is blocked, below it discharging is blocked, with 0.05 V/cell re-entry hysteresis. Without a valid projection the configured limits are kept unchanged - these are reports, not cutoffs, and the BMS's own protection does the disconnecting. `FALLBACK_BMS_CABLE_WARN_MINUTES` delays the BmsCable warning while the fallback serves, so short self-healing Bluetooth blips do not raise warnings, and the stale-data InternalFailure alarm is suppressed meanwhile. If the fallback device stops answering too, nothing is invented: the ordinary disconnect handling takes over exactly as if the feature were not configured. The wrapper also exposes the resolved fallback sensor's service name via `get_paired_sensor_device()` by @cgoudie
