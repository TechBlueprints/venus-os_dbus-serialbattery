...
* Changed: dbushelper.py - Cache the `/Settings/CGwacs/BatteryLife` and `/Settings/CGwacs/Hub4Mode` reads used by Time-to-Go for 5 minutes. Each read is a recursive D-Bus introspection walk (dozens of calls) and these ESS settings change rarely, so every battery was hammering `com.victronenergy.settings` on each recalculation by @cgoudie
* Changed: dbushelper.py - Emit one batched `ItemsChanged` signal per publish cycle instead of one `PropertiesChanged` per changed path, by publishing the battery values inside a velib `ServiceContext`. Alarm writes made outside the publish cycle are still emitted immediately by @cgoudie
* Changed: dbushelper.py - Ensure loading of newest battery data if more than one duplicate exists by @lex2k0
* Changed: dbushelper.py - Extend the D-Bus write caching added with https://github.com/mr-manuel/venus-os_dbus-serialbattery/pull/397 with per-path significance thresholds, so noisy measurements (current, power, temperatures, Ah counters, Time-to-Go) are only re-published once they actually moved. The comparison base stays the last published value, so slow drift always gets through, and a 15 minute heartbeat re-publishes every path even when nothing changed. Charge/discharge limits below `/Info`, all `/Alarms` paths, `/Soc` and the cell voltages are deliberately not gated by @cgoudie
