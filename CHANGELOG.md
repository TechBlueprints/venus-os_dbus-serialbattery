...
* Added: `BleakRetryBackend` Bluetooth connection backend, built on the already-bundled `bleak-retry-connector`. It resolves the device from the BlueZ cache and only scans when that misses, closes stale connections before connecting, and retries with error-classified backoff. In a 40 h production soak on a Cerbo GX with two BLE batteries it produced zero unusable-device events and near-zero connection failures by @cgoudie
* Added: `BLUETOOTH_CONNECTION_MANAGER` config option (opt-in) that installs the "bleak catcher" from a bleak-connection-manager installed on the device (`BLUETOOTH_CONNECTION_MANAGER_DIR`, default `/data/bcm`; nothing is vendored, and with no install the option only says so once) before any BLE library is imported: every bleak client the process builds is routed through claim-aware adapter selection over the `BLUETOOTH_ADAPTERS` entries, coordinated with other BLE services on the device through claim files under `/run/bt-claims`. Adds per-adapter link slots (`BLUETOOTH_CONNECTION_MANAGER_LINK_CAPS`), habluetooth's fast-then-medium connection parameter tuning (a no-op on Venus OS, whose Python is built without `socket.AF_BLUETOOTH`, so the BlueZ management socket cannot be opened and connection parameters stay at the kernel defaults), and an opt-in adapter-bound scanner with a silence watchdog (`BLUETOOTH_CONNECTION_MANAGER_WRAP_SCANNER`). This is the only routing/coordination path that also covers the aiobmsble drivers, whose `BaseBMS._connect` is `@final` and cannot use the connection backend seam by @cgoudie
...
* Changed: Bluetooth LE - Dropped the per-command event loop in `Syncron_Ble.send_data()`, which wrapped every BMS command in `asyncio.run()` just to schedule a coroutine on the BLE thread's existing loop. Verified on a Cerbo GX: sustained driver CPU dropped from 4.7% to 2.7% together with a driver-side fix by @cgoudie
* Changed: Bluetooth LE - Close stale LE links with `dbus-send` instead of `bluetoothctl` in the generated run script. The interactive `bluetoothctl` client can segfault on some GX hardware (observed as a SIGSEGV on an Allwinner A20 Cerbo), silently skipping the disconnect and leaving a stale link that blocks the driver's next connection attempt. The replacement calls `org.bluez.Device1.Disconnect` on the device object of every adapter by @cgoudie
* Changed: Bluetooth LE - Guard the connection backend's connect and disconnect with outer deadlines, so no single stalled await can park the reconnect loop. An unguarded D-Bus await once left a battery silently disconnected for four hours without a log line by @cgoudie
* Changed: Bluetooth LE - Honor a per-device hold flag file (`/data/tmp/ble-hold-<mac>`): while it exists no connection attempts are made for that battery, giving a degraded BMS radio extended quiet without taking the driver and its dbus service down, which would make DVCC raise alarms across the bank. Flags written by automatic recovery expire on their own after 20 minutes; operator-written flags persist until removed by @cgoudie
* Changed: Bluetooth LE - Paced BLE reconnect attempts with a 1 s / 3 s / 6 s ramp settling at 6 s instead of retrying every second. The old fixed 1 s retry produced 220-attempt recovery storms that wedged adapter discovery state; a session that held for more than a minute resets the ramp by @cgoudie
* Changed: Bluetooth LE - Rebuild a wedged BLE thread in-process via `Syncron_Ble.rebuild_ble_thread()` instead of exiting the driver. Killing the process leaves BlueZ in a dirty state, repeated restarts degrade the stack further and the inverter raises `BMS connection lost` while the dbus service is gone; a generation guard retires the old thread at its next loop iteration by @cgoudie
* Changed: Bluetooth LE - Rebuild the GATT tree and retry when subscribing finds the notify characteristic missing. BlueZ can report a device's services as resolved while its own view of the GATT tree is still incomplete, so `start_notify()` raises `BleakCharacteristicNotFoundError` for a characteristic the battery genuinely has. The connection was given up and the driver reconnected endlessly against working hardware; it now rebuilds the tree from live BlueZ state and tries again before giving up by @cgoudie
