...
* Changed: aiobmsble - Resolve the BLE device from the BlueZ cache instead of starting a new discovery on every refresh poll, which failed with `org.bluez.Error.InProgress`; give the first connect a 40 s budget so sparsely advertising packs are not missed; and never block the polling thread waiting on the BMS, so a slow or absent battery can no longer leave the D-Bus service registered but mute; and pace reconnects to a device that cannot be reached, which was previously retried every second indefinitely at four BlueZ connect attempts each - a battery pinned to an adapter that has been unplugged now backs off to one attempt a minute and logs once per outage instead of twice a second by @cgoudie
