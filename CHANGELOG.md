...
* Changed: dbus-serialbattery.py - Publish the battery values right after the service is registered. `setup_vedbus()` adds every value path with `None` and the first real values were only written on the first poll tick, so a freshly registered service advertised `None` on `/Dc/0/Voltage`, `/Soc` and friends for a full poll interval (1 s for BLE batteries). Consumers that recompute when a monitored value changes sample exactly that leading edge, which made `dbus-aggregate-batteries` crash with `TypeError: unsupported operand type(s) for +=: 'int' and 'NoneType'` by @cgoudie
