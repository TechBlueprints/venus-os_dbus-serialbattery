...
* Added: fallback mode (`FALLBACK_SENSOR_DBUS_DEVICE` and `FALLBACK_STOP_MINUTES`) to keep operating from a fallback device, e.g. a SmartShunt, while the connection to the BMS is lost: voltage, current, temperature and SoC are read live from the fallback device, per-cell voltages are projected from the live shunt voltage (the disconnect snapshot shifted by the pack delta, published in place of the last-known cells), remaining capacity, consumed amp hours, time-to-go and battery state are derived from the live fallback values, and only the values the fallback cannot provide are held at the last BMS read instead of resetting them and going into an error state. With the dedicated `FALLBACK_SAFE_CELL_VOLTAGE_MIN/MAX` safe zone configured (set from the battery manufacturer's spec; used ONLY in fallback mode, deliberately not shared with any other voltage option), charge and discharge follow the projected cell extremes: inside the zone both are allowed with the last known limits, above it charging is blocked, below it discharging is blocked, with 0.05 V/cell re-entry hysteresis. Without a configured zone or a valid projection, charging is blocked entirely and discharging stays allowed. Fallback mode continues indefinitely while the fallback delivers live values. If the fallback goes dark too, the BmsCable alarm is raised immediately, and `FALLBACK_STOP_MINUTES` bounds further operation: after that many minutes without live fallback data the BMS is treated as ordinarily disconnected (values reset, stock disconnect handling) until it recovers — 0 (default) holds the last values until either device returns. Fallback mode engages unconditionally on disconnect (the BMS's own cell-level protection acts independently of the driver), and `FALLBACK_BMS_CABLE_WARN_MINUTES` delays the BmsCable warning while the fallback is actively serving, so short self-healing Bluetooth blips do not raise warnings by @cgoudie
