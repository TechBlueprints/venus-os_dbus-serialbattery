...
class HumsiENK_Syncron_Ble(Syncron_Ble):
    ...
    than 10 s, so this subclass adds:
      ...
      - live `connected` tracking: set True after the backend establishes
        the connection (however long that took), False when the link drops
    ...
    Connection mechanics (device resolution, adapter selection/rotation,
    scan coordination, retries, escalation) are delegated to the configured
    BLE connection backend (BLUETOOTH_CONNECTION_BACKEND) — use BCMBackend
    on hosts where multiple BLE services contend for adapters.
    """
                ...
                if self.client:
                    while self.client.is_connected and self.main_thread.is_alive():
                        if self.connected and (time.time() - self._watchdog_last_fed) > self.WATCHDOG_TIMEOUT:
                            logger.error(f"HumsiENK: no BLE data for {self.WATCHDOG_TIMEOUT:.0f}s despite active link — dropping to force a reconnect")
                            break
...
<<<<<<< HEAD
                    ...
                    cmd_name = {0x00: "handshake", 0x20: "status", 0x21: "battery_info", 0x22: "cell_voltages", 0x58: "config", 0xF5: "version"}.get(
...
<<<<<<< HEAD
                ...
                ble_connected = self.ble_handle and hasattr(self.ble_handle, "connected") and self.ble_handle.connected

                # If no data for >10s, re-send handshake (0x00) to re-initialize
                # the BMS notification stream after a daemon-thread
                # auto-reconnection.  Retry every 30 s while starved: a single
                # resend can be lost if it lands while the BMS is still
                # settling after the reconnect (observed live — link up, one
...
<<<<<<< HEAD
            ...
            idx += 2  # cell OVP recovery (mV) - not used
            idx += 2  # OVP delay - not used

            cell_uvp_mv = int.from_bytes(data[idx : idx + 2], "little", signed=False)
            cell_uvp = cell_uvp_mv / 1000.0  # Convert to volts
            idx += 2

            idx += 2  # cell UVP recovery (mV) - not used
            idx += 2  # UVP delay - not used
            ...
            idx += 2  # charge OCP delay - not used
            ...
            idx += 2  # discharge OCP1 delay - not used
