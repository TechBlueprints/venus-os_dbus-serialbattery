#!/usr/bin/env python3
# Minimal LLT/JBD BLE probe without D-Bus registration
...
# Ensure vendor libs in ext/ are available (bleak, etc.)
BASE_DIR = os.path.dirname(os.path.dirname(__file__))
EXT_DIR = os.path.join(BASE_DIR, "ext")
if EXT_DIR not in sys.path:
    sys.path.insert(1, EXT_DIR)

from bms.lltjbd_ble import LltJbd_Ble


def main(addr: str, seconds: int = 60) -> int:
    logging.basicConfig(level=logging.DEBUG, format="%(levelname)s:%(name)s:%(message)s")
    addr = addr.strip()
    port = "ble_" + addr.replace(":", "").lower()
    logging.info(f"Starting JBD BLE probe for {addr} (port={port}) for ~{seconds}s")

    bat = LltJbd_Ble(port=port, baud=9600, address=addr)

    if not bat.test_connection():
        logging.error("test_connection() returned False")
        return 2

    # Give the background loop a head start
    sleep(3)
    ...
    while time() - started < seconds:
        try:
            ...
            if ok_cells and bat.cells:
                ...
                logging.info(
                    f"CELLS ok: count={bat.cell_count} populated={populated} first={first}V"
                )
            else:
                logging.debug("CELLS read failed")
        except KeyboardInterrupt:
            break
        except Exception:
            # keep probing on intermittent errors
            pass
        sleep(2)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: jbd_ble_probe.py <BLE_ADDR> [seconds]", file=sys.stderr)
        sys.exit(64)
    addr = sys.argv[1]
