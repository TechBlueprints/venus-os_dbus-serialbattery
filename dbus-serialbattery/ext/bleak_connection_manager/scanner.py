...
async def _poll_bluez_cache(
    address: str,
    wait_seconds: float,
) -> BLEDevice | None:
    """Poll the BlueZ D-Bus cache instead of running our own scan.

    Used when an external process is already running ``StartDiscovery``
    on the adapter: its results land in the shared BlueZ D-Bus object
    tree, so polling the cache finds the device without ever scanning
    ourselves (and without fighting the external scan for the adapter's
    discovery session).
...
async def find_device(
    ...
    **Cache-first approach** (avoids ``StartDiscovery`` when possible):
    ...
    2. Only if the cache misses do we actually call
       ``BleakScanner.find_device_by_address()`` (which triggers
       ``StartDiscovery``), rotating adapters between attempts.
        ...
        if IS_LINUX and not passive:
            try:
                ...
                if scan_state == AdapterScanState.EXTERNAL_SCAN:
                    ...
                    cached = await _poll_bluez_cache(
                        address, timeout,
                    )
                    if cached is not None:
                        _LOGGER.info(
...
async def discover(
    ...
    **scanner_kwargs: Any,
    ...
    Drop-in replacement for ``BleakScanner.discover()`` with adapter
    rotation and InProgress retry built in.
